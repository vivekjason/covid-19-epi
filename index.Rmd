...
clusters <- read_csv("P:/COVID-19/R0 Malaysia/data/cluster data/clusters_dashboard.csv", 
    col_types = cols(no = col_number(), bangau = col_number(), 
        ...
        bukit_besi=col_number(), pengkalan_chepa=col_number()))
...
df$ins <- df$dti+df$tembok+df$benteng_pk+df$penjara_reman+df$penjara_jawi+df$kepayan+df$penjara_tapah+
  ...
  df$tembok_sgudang +  df$tembok_taiping + df$bukit_besi + df$pengkalan_chepa
...
new <- new %>%
  mutate(daily_ma01 = rollmean(new_cases, k = 5, fill = "extend"))
...
#load data
Global_Mobility_Report_3_ <- read_csv("P:/COVID-19/R0 Malaysia/data/mobility/Global_Mobility_Report_210210.csv", 
...
perlis_r0_df <- perlis_r0_df %>%
  mutate(daily_ma01 = rollmean(case, k = 5, fill = "extend"))
